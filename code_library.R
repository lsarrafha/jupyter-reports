...
apply_characteristic_direction <- function(dataframe, design_dataframe, constant_threshold=10) {
	...
	# Get dataframes
	experimental_dataframe <- log_dataframe[,rownames(design_dataframe)[design_dataframe[,'samples'] == 1]]
	control_dataframe <- log_dataframe[,rownames(design_dataframe)[design_dataframe[,'controls'] == 1]]

	# Get variable genes
	experiment_variable_genes <- names(which(diag(var(t(experimental_dataframe))) > constant_threshold))
	control_variable_genes <- names(which(diag(var(t(control_dataframe))) > constant_threshold))

	# Get common genes
	common_genes <- intersect(experiment_variable_genes, control_variable_genes)

	# Filter dataframes
	experimental_dataframe_filtered <- experimental_dataframe[common_genes,]
	control_dataframe_filtered <- control_dataframe[common_genes,]
